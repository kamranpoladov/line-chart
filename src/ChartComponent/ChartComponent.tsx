...
const ChartComponent = ({ defaultRange, data }: PlotProps) => {
  ...
  const filteredData = useMemo(() => filterData(range, data, isMobile), [
    range,
    data,
    isMobile,
  ]);
  ...
  const handleRangeChange = (range: Range = defaultRange, index: number) => {
    setRange(range);
  ...
  useEffect(() => {
    ...
    const yAxisGenerator: d3.ScaleLinear<number, number> = d3
      .scaleLinear()
      .domain([
        minForRange(filteredData, range),
        maxForRange(filteredData, range) * 1.1,
      ])
    ...
    d3.select(areaPathRef.current)
      ...
      .attr(
        ...
        d3
          .area<DataPoint>()
          .defined(
            (d) => d.date <= range.rangeRight && d.date >= range.rangeLeft
          )
                  ...
                  handleRangeChange(
                    {
                      ...
                      rangeRight: defaultRange.rangeRight,
                    },
                    index
                  )
                }
